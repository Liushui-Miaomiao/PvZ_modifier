	...
	switch(command){
			...
			switch(command){
				case '1':
					if(openMod()){
						...
						cheatMsg(writeMemory_2(hProcess,0x006A9EC0,0x768,0x5560,Sun,4),"阳光");
					}
					break;
				case '2':
					if(openMod()){
						...
						cheatMsg(writeMemory_2(hProcess,0x006A9EC0,0x82C,0x28,Money,4),"金钱");
					}
					break;
				case '3':
					if(openMod()){
						cheatMsg(writeMemory(hProcess,0x00487296,0x1477,2),"卡牌无冷却");
					}
					break;
				case '4':
					if(openMod()){
						if(backStage==1){
							cheatMsg(writeMemory(hProcess,0x0054EBEF,0xC3,1),"取消后台");
						}else{
							cheatMsg(writeMemory(hProcess,0x0054EBEF,0x57,1),"恢复后台");
						}
						backStage=!backStage;
					}
					break;
				case '5':
					if(openMod()){
						...
						if(Card<1)Card=1;
						if(Card>10)Card=10;
						cheatMsg(writeMemory_3(hProcess,0x006A9EC0,0x768,0x144,0x24,Card,4),"卡槽格数");
					}
					break;
				case '6':
					if(openMod()){
						cheatMsg(writeMemory(hProcess,0x0043158F,0xEB,1),"自动收集开启");
					}
					break;
				default :
					noOperation();
			...
			switch(command){
				case '1':
					if(openMod()){
						...
						Checkpoint-=10;
						cheatMsg(writeMemory_2(hProcess,0x006A9EC0,0x82C,0x24,Checkpoint,4),"冒险模式关卡");
					}
					break;
				case '2':
					if(openMod()){
						...
						cheatMsg(writeMemory_1(hProcess,0x006A9EC0,0x7F8,MiniGame,4),"小游戏跳关");
					}
					break;
				default :
					noOperation();
		...
		case 'c':
			...
			switch(command){
				case '1':
					if(openMod()){
						if(anyPosition==0){
							cheatMsg(writeMemory(hProcess,0x0040FE30,0x81,1)&&writeMemory(hProcess,0x00438E40,0xEB,1)&&writeMemory(hProcess,0x0042A2D9,0x8D,1),"开启重叠放置");
						}else{
							cheatMsg(writeMemory(hProcess,0x0040FE30,0x84,1)&&writeMemory(hProcess,0x00438E40,0x74,1)&&writeMemory(hProcess,0x0042A2D9,0x84,1),"取消重叠放置");
						}
				...
				case '2':
					if(openMod()){
						cheatMsg(writeMemory(hProcess,0x0041D7D0,0xC301B0,3)&&writeMemory(hProcess,0x0040E477,0xEB,1),"紫卡种植无限制");
					}
					break;
				default :
					noOperation();
		...
		case 'd':
			...
			switch(command){
				case '1':
					if(openMod()){
						...
						for(z_i=0;z_i<1024;z_i++){
							...
							if(Z_Disappear==0)kill=writeMemory_3(hProcess,0x006A9EC0,0x768,0x90,z_i*0x15C+0x28,3,4);
						}
				...
				case '2':
					if(openMod()){
						if(little==0){
							cheatMsg(writeMemory(hProcess,0x00523ED5,0xEB,1),"开启小僵尸特效");
						}else{
							cheatMsg(writeMemory(hProcess,0x00523ED5,0x74,1),"关闭小僵尸特效");
						}
						little=!little;
					}
					break;
			...
			switch(command){
				case '1':
					if(openMod()){
						...
						scanf("%d",&Tree);
						if(Tree<0)Tree=0;
						cheatMsg(writeMemory_2(hProcess,0x006A9EC0,0x82C,0xF4,Tree,4),"智慧树高度");
					}
					break;
				case '2':
					if(openMod()){
						...
						Chocolates+=1000;
						cheatMsg(writeMemory_2(hProcess,0x006A9EC0,0x82C,0x228,Chocolates,4),"巧克力数量");
					}
					break;
				default :
					noOperation();
		...
		case 'f':
			...
			switch(command){
				case '1':
					if(openMod()){
						cheatMsg(writeMemory(hProcess,0x0044E5CC,0x0033B866,4),"花瓶透视开启");
					}
					break;
				case '2':
					if(openMod()){
						cheatMsg(writeMemory(hProcess,0x0042DF5D,0x38,1),"显示隐藏小游戏");
					}
					break;
				default :
					noOperation();
